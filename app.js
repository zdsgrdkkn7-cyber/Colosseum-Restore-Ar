...
function updateFinalCost(){
  ...
  const discount=Math.max(0,parseMoney($("#descuentoResultado")?.value));
  const final=Math.max(0,budget-discount);
  const hidden=$("#precioTrabajo");
  const display=$("#finalCostDisplay");
  if(hidden)hidden.value=formatMoneyInput(final);
  if(display)display.textContent=formatUsd(final);
}
...
function collect(){
 const d={id:state.id,createdAt:new Date().toISOString(),ratings:state.ratings,photos:state.photos,work:state.work,budgetManualOverride:state.budgetManualOverride,fields:{}};
...
function clearForm(){
 state.id=null;state.ratings={before:{},after:{}};state.photos={frontBefore:null,frontAfter:null,backBefore:null,backAfter:null};state.work={limpieza:false,bordes:false,pliegues:false,relaminado:false,prensado:false,pulido:false,lustrado:false,otro:false};state.budgetManualOverride=false;
