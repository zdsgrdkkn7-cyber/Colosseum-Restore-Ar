...
const fields=["cliente","fecha","carta","numero","set","version","idioma","estado","precio","observaciones","otroTrabajo","resultadoFrente","resultadoDorso","precioTrabajo"];
...
function collect(){
 const d={id:state.id,createdAt:new Date().toISOString(),ratings:state.ratings,photos:state.photos,work:state.work,fields:{}};
 fields.forEach(id=>{const el=$("#"+id);d.fields[id]=el?el.value:""});
 return d;
}
function apply(data){
 if(!data)return;
 state.id=data.id||null;state.ratings=data.ratings||{before:{},after:{}};state.photos=data.photos||{};state.work=data.work||{};
 fields.forEach(id=>{
   const el=$("#"+id);
   if(!el)return;
...
fields.forEach(id=>{const el=$("#"+id);if(el)el.addEventListener("input",autosaveDraft)});
...
function initControls(){
 const saveBtn=$("#saveBtn"),newBtn=$("#newBtn"),pdfBtn=$("#pdfBtn"),historyBtn=$("#historyBtn"),closeHistory=$("#closeHistory"),
       remitoBtn=$("#remitoBtn"),selectAllReports=$("#selectAllReports");
 ...
 if(selectAllReports)selectAllReports.onclick=()=>{
   ...
   // Re-render is simplest and keeps every checkbox synchronized.
...
document.addEventListener("DOMContentLoaded",()=>{
 ...
 try{const d=JSON.parse(localStorage.getItem("cr_draft")||"null");if(d)apply(d)}catch(e){console.error("No se pudo recuperar borrador",e)}
 fitMobile();
