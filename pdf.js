...
function renderReportPage(doc,rawReport,assets){
  ...
  doc.setFontSize(6.6);
  doc.setTextColor(...cream);
  const baseY=resultDiscountPercent>0?597.5:600;
  doc.text("Precio base presupuestado",14,baseY);
  doc.text(pdfMoney(budgetAmount),194,baseY,{align:"right"});

  if(resultDiscountPercent>0){
    const discountAmount=budgetAmount*(resultDiscountPercent/100);
    doc.setTextColor(232,48,48);
    doc.text(`Descuento por resultado (${String(resultDiscountPercent).replace(".",",")}%)`,14,602.5);
    doc.text(`- ${pdfMoney(discountAmount)}`,194,602.5,{align:"right"});
  }
