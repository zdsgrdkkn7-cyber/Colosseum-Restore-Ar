...
function drawCenteredFrameTitle(doc,x,y,w,title){
  ...
  const pad=4;
  doc.setFillColor(...black);
  doc.rect(cx-tw/2-pad,y-2.4,tw+pad*2,5.2,"F");
  doc.setTextColor(...amber);
...
async function generatePdf(){
  ...
  doc.setFontSize(18);
  doc.text("REPORTE DE RESTAURACION",105,25,{align:"center"});
  for(let i=0;i<6;i++) drawPdfPokeball(doc,91.75+i*5.3,35.5,1.8,true);
  ...
  // EDGE / GENERAL
  drawBattleFrame(doc,8,460,194,42); drawCenteredFrameTitle(doc,8,460,194,"CANTO / BORDE + GENERAL");
  ...
  drawBattleFrame(doc,110,510,92,72); drawCenteredFrameTitle(doc,110,510,92,"OBSERVACIONES");
  ...
  // PRICE
  drawBattleFrame(doc,8,590,194,18); drawCenteredFrameTitle(doc,8,590,194,"PRECIO DEL TRABAJO");
