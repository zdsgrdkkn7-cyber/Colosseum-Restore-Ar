...
function renderReportPage(doc,rawReport,assets){
  ...
  workLabels.forEach(([k,label],i)=>{
    ...
    const clipped=doc.splitTextToSize(shown,61)[0];
    doc.text(clipped,19,yy);
    doc.setTextColor(...amber);
    doc.text("U$ 5",94,yy,{align:"right"});
  });
  ...
  const workSubtotal=getReportWorkCount(report)*5;
  ...
  doc.setTextColor(...cream);
  doc.text("Trabajos realizados",14,594);
  doc.text(pdfMoney(workSubtotal),194,594,{align:"right"});

  doc.setTextColor(...amber);
  doc.text("Factor de responsabilidad",14,598.5);
  doc.setTextColor(...cream);
  doc.text(factorInfo.label,194,598.5,{align:"right"});

  doc.setTextColor(...cream);
  doc.text("Precio base presupuestado",14,603);
  doc.text(pdfMoney(budgetAmount),194,603,{align:"right"});

  if(resultDiscountPercent>0){
    const discountAmount=budgetAmount*(resultDiscountPercent/100);
    doc.setTextColor(232,48,48);
    doc.text(`Descuento por resultado (${String(resultDiscountPercent).replace(".",",")}%)`,14,607.5);
    doc.text(`- ${pdfMoney(discountAmount)}`,194,607.5,{align:"right"});
  }

  doc.setDrawColor(...cream);doc.setLineWidth(.28);
  doc.line(116,609.5,194,609.5);
  doc.setTextColor(...amber);doc.setFontSize(8.7);
  doc.text("COSTO FINAL",14,613);
  doc.text(pdfMoney(finalAmount),194,613,{align:"right"});
}
