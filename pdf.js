...
function renderReportPage(doc,rawReport,assets){
  ...
  // COST — DESGLOSE COMPLETO
  drawBattleFrame(doc,8,586,194,28);
  drawCenteredFrameTitle(doc,8,586,194,"COSTO DE RESTAURACION");

  const selectedWorks=[
    ["limpieza","Limpieza"],
    ["bordes","Limpieza de bordes"],
    ["pliegues","Reparacion pliegues"],
  ...
  // Left half: every work actually performed, U$5 each.
  doc.setFont("courier","bold");
  doc.setFontSize(5.2);
  doc.setTextColor(...amber);
  doc.text("TRABAJOS REALIZADOS",14,593);

  const leftCols=[14,61];
  selectedWorks.forEach(([k,label],idx)=>{
    const col=idx<4?0:1;
    const row=idx%4;
    const yy=598+row*3.7;
    let shown=(k==="otro" && f.otroTrabajo)?`Otro: ${pdfSafeText(f.otroTrabajo)}`:label;
    shown=doc.splitTextToSize(shown,34)[0];
    doc.setTextColor(...cream);
    doc.text(shown,leftCols[col],yy);
    doc.setTextColor(...amber);
    doc.text("U$ 5",leftCols[col]+39,yy,{align:"right"});
  });

  if(!selectedWorks.length){
    doc.setTextColor(...cream);
    doc.text("Sin trabajos seleccionados",14,599);
  }

  // Right half: factor and complete price calculation.
  const lx=112,rx=194;
  doc.setFontSize(5.5);
  doc.setTextColor(...cream);
  doc.text("Subtotal trabajos",lx,593);
  doc.text(pdfMoney(workSubtotal),rx,593,{align:"right"});

  doc.setTextColor(...amber);
  doc.text("Factor de responsabilidad",lx,598);
  doc.setTextColor(...cream);
  doc.text(factorInfo.label,rx,598,{align:"right"});

  doc.text("Precio base presupuestado",lx,603);
  doc.text(pdfMoney(budgetAmount),rx,603,{align:"right"});

  if(resultDiscountPercent>0){
    const discountAmount=budgetAmount*(resultDiscountPercent/100);
    doc.setTextColor(232,48,48);
    doc.text(`Descuento resultado (${String(resultDiscountPercent).replace(".",",")}%)`,lx,608);
    doc.text(`- ${pdfMoney(discountAmount)}`,rx,608,{align:"right"});
  }

  doc.setDrawColor(...cream);doc.setLineWidth(.25);
  doc.line(lx,609.5,rx,609.5);
  doc.setTextColor(...amber);doc.setFontSize(7.6);
  doc.text("COSTO FINAL",lx,613);
  doc.text(pdfMoney(finalAmount),rx,613,{align:"right"});
}
...
function renderDisclaimerPage(doc){
  ...
  // Intentionally understated: one frame, no repeated logo/header.
  drawBattleFrame(doc,12,18,186,584);
  drawCenteredFrameTitle(doc,12,18,186,"DISCLAIMER");
  ...
  doc.setTextColor(...amber);doc.setFontSize(6.3);
