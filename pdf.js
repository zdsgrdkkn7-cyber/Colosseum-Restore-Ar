...
function renderReportPage(doc,rawReport,assets){
  ...
  // Height grows with content, so nothing can fall outside the frame.
  const workRows=Math.max(1,selectedWorks.length);
  const extraRows=3+(resultDiscountPercent>0?1:0)+1; // subtotal, factor, base, optional discount, final
  const costBoxH=14 + workRows*4.3 + extraRows*5.0 + 8;
  const costBoxY=584;
