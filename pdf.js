...
function renderReportPage(doc,rawReport,assets){
  ...
  // COST
  drawBattleFrame(doc,8,590,194,22);
  ...
  const resultDiscount=parsePdfMoney(f.descuentoResultado);
  const finalAmount=parsePdfMoney(f.precioTrabajo || Math.max(0,budgetAmount-resultDiscount));

  doc.setFontSize(6.8);
  doc.setTextColor(...cream);
  doc.text("Precio base presupuestado",14,598);
  doc.text(pdfMoney(budgetAmount),194,598,{align:"right"});
  doc.setTextColor(232,48,48);
  doc.text("Descuento por resultado",14,603);
  doc.text(`- ${pdfMoney(resultDiscount)}`,194,603,{align:"right"});
  doc.setDrawColor(...cream);doc.setLineWidth(.3);doc.line(120,605,194,605);
  doc.setTextColor(...amber);doc.setFontSize(9.5);
  doc.text("COSTO FINAL",14,609);
  doc.text(pdfMoney(finalAmount),194,609,{align:"right"});
}
...
function renderBudgetConditionsPage(doc,assets){
  const cream=[242,242,235],amber=[242,165,26],red=[232,48,48],muted=[150,150,145];
  const W=210,H=620;
  paintPdfBackground(doc,W,H);
  ...
  doc.setTextColor(...cream);doc.setFontSize(8.5);
  doc.text("CONDICIONES DEL PRESUPUESTO",105,42,{align:"center"});

  // Factor table.
  drawBattleFrame(doc,8,64,194,155);
  drawCenteredFrameTitle(doc,8,64,194,"FACTOR DE RESPONSABILIDAD");
  const rows=[
    ["Hasta U$50","x0,50"],
    ["U$51 - 100","x0,75"],
    ["U$101 - 250","x1,00"],
  ...
  doc.setFontSize(7.6);
  doc.setTextColor(...amber);
  doc.text("VALOR DECLARADO",30,82);
  doc.text("FACTOR",178,82,{align:"right"});
  doc.setDrawColor(...muted);doc.line(22,86,188,86);

  rows.forEach((row,i)=>{
    const y=99+i*12;
    doc.setTextColor(...cream);
    doc.text(row[0],30,y);
    doc.text(row[1],178,y,{align:"right"});
    doc.setDrawColor(65,65,65);doc.setLineWidth(.2);doc.line(22,y+4,188,y+4);
  });

  doc.setTextColor(...amber);doc.setFontSize(7);
  doc.text("Cada intervencion: U$ 5  |  Precio minimo de restauracion: U$ 15",105,212,{align:"center"});

  function section(y,title,text,height){
    drawBattleFrame(doc,8,y,194,height);
    drawCenteredFrameTitle(doc,8,y,194,title);
    doc.setTextColor(...cream);doc.setFontSize(7.1);
    const lines=doc.splitTextToSize(text,174);
    doc.text(lines,18,y+17,{baseline:"top"});
  }

  section(
    232,"COTIZACION DEL SERVICIO",
  ...
  section(
    430,"RESULTADO DE LA RESTAURACION",
  ...
  drawBattleFrame(doc,8,569,194,38);
  drawCenteredFrameTitle(doc,8,569,194,"ACEPTACION");
  doc.setTextColor(...cream);doc.setFontSize(6.8);
  const acceptance="La aceptacion del presupuesto implica la conformidad del cliente con los trabajos propuestos, el precio presupuestado, el valor declarado de la carta y los riesgos inherentes al proceso de restauracion.";
  doc.text(doc.splitTextToSize(acceptance,174),18,585,{baseline:"top"});
}
...
async function generateBudgetPdf(){
  ...
  renderBudgetConditionsPage(doc,assets);
